...
OLD = """You are a deterministic, honest and truthful assistant program. You are being asked to generate a single multiple choice question based off of a given prompt.
...
class Llama:
    """A singleton class for interacting with the Llama model

    ```py
    llama = Llama.instance(
        system_prompt=SYSTEM_PROMPT, # default is set in the class
        MAX_TOKENS=2048, # default is 4096
    )
    streamer = llama.generate([
    ...
    def instance(cls, **kwargs):
        """A singleton constructor for Llama
        ...
        **kwargs:
        ...
        Returns:
            Llama: the Llama instance that was created
        """
        ...
        # kwarg settings for all self.CAPITAL settings
        for k in dir(cls):
            if k.isupper():
                setattr(cls, k, kwargs.get(k, getattr(cls, k)))
    ...
    def generate(self, prompt: list[dict[str, str]]):
        """Run a streaming generation on the LLM

        Args:
            prompt `(list[dict[str, str]])`: a list of objects with the shape 
        ...
        Raises:
            RuntimeError: if the model is not instantiated yet
            RuntimeError: if the tokenizer is not instantiated yet

        Returns:
            `TextIteratorStreamer`: a generator that yields new text as it is generated by the LLM.\
            example:
            ```py
            streamer = llama.generate([...])
